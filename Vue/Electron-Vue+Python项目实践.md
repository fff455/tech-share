...
* 而对于``electron``应用而言，其本身就是一个能够正常安装卸载的应用，这在其支持的操作系统中均能够满足。不过在最初始的情况下，这个安装卸载就是一个纯粹的安装与卸载。简单的一个体现就是安装不需要选择路径，以及在Windows中仅仅安装至了单个用户。对于这些功能的实现，仅需在``package.json``中进行配置即可。同时，配置文件中的版本``version``也直接对应于安装运行后的在``Windows``中注册的版本
