...
注意生成这个函数的时候调用了 useCallback，依赖项为 key，即只有在 key 发生变化的时候才会重新生成 revalidate 函数。 我们先聚焦于主要流程，此时 shouldDeduping === false。
...
即检验 CONCURRENT\_PROMISES 里有没有 key 所对应的进行中的请求。
